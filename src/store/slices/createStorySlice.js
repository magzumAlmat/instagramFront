...
export const {authorize, logout, editVar,createStoryReducer,getUsersPostsReducer} = createStorySlice.actions;

export const createStoryFunc = (formData) => (dispatch) => {
    const token = localStorage.getItem("token");
    ...
    try{
     axios.post(`${END_POINT}/api/story`, formData, {
                headers: {
...
export const getUsersPostsAction = () => async (dispatch) => {
    ...
    try {
      const response = await axios.get(`${END_POINT}/api/story/user/${decodedToken.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'multipart/form-data',
        },
